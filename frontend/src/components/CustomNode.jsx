...
const CustomNode = ({ data, selected }) => {
    ...
    return (
        <div className={`
            group relative min-w-[280px] transition-all duration-300 transform
        ...
        `}>
            ...
            `}>
                {/* Header Section */}
                <div className="flex items-center gap-3 p-4 border-b border-black/[0.03]">
                    <div className={`
                    ...
                    <div className="flex-1 min-w-0">
                        <h3 className="text-[14px] font-bold text-[#0f172a] truncate leading-tight">
                            {data.label}
                        </h3>
                        <div className="flex items-center gap-2 mt-0.5">
                            <span className={`text-[10px] font-bold uppercase tracking-wider ${categoryColors[data.category] || 'text-[#94a3b8]'}`}>
                                {data.category}
                            </span>
                        </div>
                    </div>
                    {isError && <AlertCircle size={16} className="text-[#dc2626]" />}
                </div>

                {/* Status Bar */}
                <div className="px-4 py-3 flex items-center justify-between bg-[#f8fafc]/50 rounded-b-xl border-t border-black/[0.02]">
                    <div className="flex items-center gap-2">
                        {isRunning ? (
                            <Loader2 size={12} className="text-[#2563eb] animate-spin" />
                        ) : (
                            <div className={`w-2 h-2 rounded-full ring-2 ring-black/5 ${isSuccess ? 'bg-[#16a34a]' :
                                isError ? 'bg-[#dc2626]' : 'bg-[#94a3b8]'
                                }`} />
                        )}
                        <span className={`text-[11px] font-bold uppercase tracking-wide ${isRunning ? 'text-[#2563eb]' :
                    ...
                    {isSuccess && (
                        <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-[#16a34a] text-white">
